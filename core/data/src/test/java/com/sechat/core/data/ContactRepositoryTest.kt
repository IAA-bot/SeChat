...
class ContactRepositoryTest {
    ...
    @Test
    fun `add contact stores and returns id`() =
        runBlocking {
    ...
    @Test
    fun `add duplicate public key returns existing id`() =
        runBlocking {
    ...
    @Test
    fun `get non-existent contact returns null`() =
        runBlocking {
    ...
    @Test
    fun `set online updates status and lastSeen`() =
        runBlocking {
            val id = repository.addContact("Alice", ByteArray(32) { 1 })
...
class FakeContactDao : ContactDao {
    ...
    override suspend fun getByPublicKey(publicKey: ByteArray): ContactEntity? = contacts.find { it.publicKey.contentEquals(publicKey) }
