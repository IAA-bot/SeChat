...
class WebRTCManager(private val context: Context) {
    ...
    fun createOffer(callback: SignalingCallback) {
        ...
        thread.execute {
            ...
            val dc = peerConnection?.createDataChannel("sechat", DataChannel.Init().apply {
                ordered = true
                negotiated = false
            })
            ...
            peerConnection?.createOffer(object : org.webrtc.SdpObserver {
                override fun onCreateSuccess(desc: SessionDescription) {
                    peerConnection?.setLocalDescription(object : org.webrtc.SdpObserver {
                        ...
                        override fun onSetFailure(msg: String) { _state.value = WebRTCState.FAILED }
                        override fun onCreateSuccess(desc: SessionDescription) {}
                ...
                override fun onSetSuccess() {}
                override fun onSetFailure(msg: String) { _state.value = WebRTCState.FAILED }
                override fun onCreateFailure(msg: String) { _state.value = WebRTCState.FAILED }
            }, constraints)
        }
    }

    fun receiveOffer(sdp: String, callback: SignalingCallback) {
        signalingCallback = callback
        thread.execute {
            peerConnection = createPeerConnection()
